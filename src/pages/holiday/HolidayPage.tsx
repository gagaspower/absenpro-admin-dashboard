...
import { HolidayFormDrawer } from "@/components/holiday/HolidayFormDrawer"
import { PeriodeCombobox } from "@/components/holiday/PeriodeCombobox"
import {
  deleteHoliday,
  deleteMultipleHolidays,
  fetchHoliday,
  forceDeleteHoliday,
  forceDeleteMultipleHolidays,
  restoreHoliday,
  restoreMultipleHolidays,
  type HolidayStatusFilter,
} from "@/services/holiday/holiday.service"
import type { HolidayRow } from "@/types/holiday/holiday.types"
...
export function HolidayPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setError("Gagal memuat data hari libur. Coba lagi.")
        setRows([])
        setTotal(0)
      } finally {
  ...
  }, [page, perPage, debouncedSearch, statusFilter, periode, refreshKey])

  const totalPages = Math.max(1, Math.ceil(total / perPage))
  const hasNoData = !isLoading && !error && rows.length === 0

  const hasActiveFilter =
    Boolean(debouncedSearch) || statusFilter !== "active" || Boolean(periode)

  const showEmptyState = hasNoData && !hasActiveFilter
  ...
  const openEditDrawer = useCallback((row: HolidayRow) => {
    setEditingRow(row)
    setDrawerOpen(true)
  }, [])
  ...
  function rowActions(row: HolidayRow): RowAction[] {
    return [
      {
        key: "edit",
        label: "Edit",
        ...
        onClick: () =>
          setConfirmState({
            ...
            onConfirm: async () => {
              setIsActionLoading(true)
              try {
                await forceDeleteHoliday(row.id)
                setSelectedIds((prev) => {
                  const next = new Set(prev)
                  next.delete(row.id)
                  return next
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-[#EAEAEA] bg-[#F7FCFA] hover:bg-[#F7FCFA]">
                      ...
                      <TableHead className="text-[#374957]">Tanggal</TableHead>
                      <TableHead className="text-[#374957]">
                        Deskripsi
                      </TableHead>
                      <TableHead className="text-[#374957]">Berulang</TableHead>
                    ...
                    {isLoading ? (
                      ...
                      rows.map((row) => (
                        <TableRow key={row.id} className="border-[#EAEAEA]">
                          ...
                          <TableCell className="whitespace-nowrap text-[#374957]">
                          ...
                          <TableCell>
                            <span
                              className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                                row.is_recurring
                                  ? "bg-[#E9FBF6] text-[#0F9D74]"
                                  : "bg-gray-100 text-gray-500"
                              }`}
                            >
                              {row.is_recurring ? "Ya" : "Tidak"}
                            </span>
                          </TableCell>
                          <TableCell>
